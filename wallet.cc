...
std::vector<std::pair<WalletSecret, int>> Wallet::ReplaceWebcash(absl::Time timestamp, std::vector<WalletOutput>& inputs, const std::vector<std::pair<WalletSecret, Amount>>& outputs)
{
    ...
    for (const WalletOutput& webcash : inputs) {
        if (!webcash.secret) {
            std::cerr << "Unable to replace output without corresponding secret: " << to_string(PublicWebcash(webcash.amount, webcash.hash)) << std::endl;
            return {};
        }
        if (webcash.amount.i64 < 1) {
            std::cerr << "Invalid amount for replacement intput: " << to_string(PublicWebcash(webcash.amount, webcash.hash)) << std::endl;
        }
        if (webcash.spent) {
            std::cerr << "Replacement intput already spent: " << to_string(PublicWebcash(webcash.amount, webcash.hash)) << std::endl;
            return {};
        }
        in.push_back(std::string(to_string(SecretWebcash(webcash.amount, webcash.secret->secret)).c_str()));
    ...
    for (const std::pair<WalletSecret, Amount>& webcash : outputs) {
        if (webcash.second.i64 < 1) {
            std::cerr << "Invalid amount for replacement output: " << to_string(PublicWebcash(SecretWebcash(webcash.second, webcash.first.secret))) << std::endl;
            return {};
        }
        out.push_back(std::string(to_string(SecretWebcash(webcash.second, webcash.first.secret)).c_str()));
    ...
    for (const std::pair<WalletSecret, Amount>& webcash : outputs) {
        ...
        int id = AddOutputToWallet(timestamp, PublicWebcash(SecretWebcash(webcash.second, webcash.first.secret)), webcash.first.id, false);
        if (!id) {
            std::cerr << "Error creating database record for replacement output: " << to_string(PublicWebcash(SecretWebcash(webcash.second, webcash.first.secret))) << std::endl;
            continue;
        }
...
bool Wallet::Insert(const SecretWebcash& sk, bool mine)
{
    ...
    SecretWebcash change(sk.amount, wchange.secret);
